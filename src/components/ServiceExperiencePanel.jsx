...
function readProjects() {
  try {
    return JSON.parse(localStorage.getItem("dpg_projects") || "[]");
  } catch {
    return [];
  }
}
        ...
        >
          <div style={{ width: "min(620px, 94vw)", maxHeight: "86vh", overflowY: "auto", background: "#fff", borderRadius: 16, padding: 22 }}>
            ...
            <div style={{ marginTop: 18, padding: 12, background: "#faf7f2", borderRadius: 10 }}>
              ...
              />
              <button
                onClick={() => { setAiPreview({ index: editing.index, before: { title: editing.title, body: editing.body }, after: { title: `${editing.title || ""} (AI 개선)`, body: `${editing.body || ""}\n${feedback || ""}` } }); }}
            ...
            {aiPreview && (
              <div style={{ marginTop: 16, padding: 14, border: "1px solid #ddd", borderRadius: 10, background: "#fff" }}>
                <div style={{ fontWeight: 800, marginBottom: 10 }}>✨ AI 개선 결과</div>
                <div style={{ fontSize: 12, color: "#777" }}>기존 내용</div>
                <div style={{ marginBottom: 10 }}>{aiPreview.before.title}<br/>{aiPreview.before.body}</div>
                <div style={{ fontSize: 12, color: "#777" }}>개선 내용</div>
                <div>{aiPreview.after.title}<br/>{aiPreview.after.body}</div>
                <div style={{ marginTop: 10, display:"flex", gap:8 }}>
                  <button
                    onClick={() => {
                      if (!draft || !aiPreview) return;
                      const index = aiPreview.index;
                      const after = aiPreview.after;
                      if (index === "hero") {
                        onDraftChange?.({
                          ...draft,
                          hero_headline: after.title,
                          hero_subcopy: after.body
                        });
                      } else {
                        const sections = [...(draft.sections || [])];
                        sections[index] = {
                      ...
                      setAiPreview(null);
                      setNotice("AI 개선 내용이 적용되었습니다.");
                      setTimeout(() => setNotice(""), 2200);
                    }}
                    style={{ padding:"8px 12px", background:"#8A6A56", color:"#fff", border:"none", borderRadius:8 }}
                  >
                    적용
                  </button>
                  <button onClick={() => setAiPreview(null)} style={{ padding: "8px 12px" }}>다시 수정</button>
                </div>
              </div>
            )}

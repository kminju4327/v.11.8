...
export default function DetailPagePreview({ draft, generatedPage, image, concept, themeColor, onRegen, regenIndex, onEditSection, onAIImproveRequest }) {
  ...
  return (
    ...
    <div style={{ maxWidth: 640, margin: "0 auto" }}>
      {/* HERO */}
      <PreviewSection idx="hero" onRegen={onRegen} loading={regenIndex === "hero"} accent={themeColor} onEdit={() => setEditing({index:"hero", section:{title:draft.hero_headline, body:draft.hero_subcopy, items:[]}})}>
        <div
          style={{
      ...
      {/* SECTIONS */}
      {sections?.map((s, i) => (
        <PreviewSection key={i} idx={i} onRegen={onRegen} loading={regenIndex === i} accent={themeColor} onEdit={() => setEditing({index:i, section:s})}>
          <div
            style={{
              borderRadius: conceptStyle.radius,
              boxShadow: conceptStyle.shadow,
